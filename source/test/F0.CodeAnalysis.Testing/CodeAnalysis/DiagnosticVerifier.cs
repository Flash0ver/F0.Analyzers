// Based on Visual Studio 16.5.5 template "Analyzer with Code Fix (.NET Standard)"
using System;
	...
	public abstract partial class DiagnosticVerifier
	{
		...
		private static void VerifyDiagnosticResults(IEnumerable<Diagnostic> actualResults, DiagnosticAnalyzer analyzer, params DiagnosticResult[] expectedResults)
		{
			...
			if (expectedCount != actualCount)
			{
				...
				throw new InvalidOperationException(String.Format("Mismatch between number of diagnostics returned, expected \"{0}\" actual \"{1}\"\r\n\r\nDiagnostics:\r\n{2}\r\n", expectedCount, actualCount, diagnosticsOutput));
			}
			...
			for (var i = 0; i < expectedResults.Length; i++)
			{
				...
				if (expected.Line == -1 && expected.Column == -1)
				{
					...
					{
						throw new InvalidOperationException(String.Format("Expected:\nA project diagnostic with No location\nActual:\n{0}",
				...
				else
				{
					...
					{
						throw new InvalidOperationException(String.Format("Expected {0} additional locations but got {1} for Diagnostic:\r\n    {2}\r\n",
				...
				{
					throw new InvalidOperationException(String.Format("Expected diagnostic id to be \"{0}\" was \"{1}\"\r\n\r\nDiagnostic:\r\n    {2}\r\n",
						expected.Id, actual.Id, FormatDiagnostics(analyzer, actual)));
				}

				if (actual.Severity != expected.Severity)
				{
					throw new InvalidOperationException(String.Format("Expected diagnostic severity to be \"{0}\" was \"{1}\"\r\n\r\nDiagnostic:\r\n    {2}\r\n",
						expected.Severity, actual.Severity, FormatDiagnostics(analyzer, actual)));
				}

				if (actual.GetMessage() != expected.Message)
				{
					throw new InvalidOperationException(String.Format("Expected diagnostic message to be \"{0}\" was \"{1}\"\r\n\r\nDiagnostic:\r\n    {2}\r\n",
		...
		private static void VerifyDiagnosticLocation(DiagnosticAnalyzer analyzer, Diagnostic diagnostic, Location actual, DiagnosticResultLocation expected)
		{
			...
			(actualSpan.Path == expected.Path || (actualSpan.Path != null && actualSpan.Path.Contains("Test0.") && expected.Path.Contains("Test."))).Should().BeTrue(
			...
			if (actualLinePosition.Line > 0)
			{
				...
				{
					throw new InvalidOperationException(String.Format("Expected diagnostic to be on line \"{0}\" was actually on line \"{1}\"\r\n\r\nDiagnostic:\r\n    {2}\r\n",
			...
			if (actualLinePosition.Character > 0)
			{
				...
				{
					throw new InvalidOperationException(System.String.Format("Expected diagnostic to start at column \"{0}\" was actually at column \"{1}\"\r\n\r\nDiagnostic:\r\n    {2}\r\n",
